...
public class UserPrinciple implements UserDetails {
    ...
    public static UserPrinciple build(AppUser user) {
        List<GrantedAuthority> authorities = user.getAppRoles().stream().map(role -> new SimpleGrantedAuthority(role.getNameRole())).collect(Collectors.toList());
        return new UserPrinciple(user.getIdUser(), user.getNameUser(), user.getPassword(), authorities);
    }
